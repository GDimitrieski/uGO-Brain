...
def station_delete_by_id(url: str, token: str, referenceId: int) -> Optional[Dict[str, Any]]:
    if not referenceId:
    ...
    try:
        request_status_response = requests.delete(f"{url}/api/stations/{referenceId}", headers=headers)
...
if __name__ == "__main__":
    ...
    station = 1
    station_delete_by_id(credentials["url"], token, station)
